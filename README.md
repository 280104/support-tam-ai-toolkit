...
- **`src/`**
  ...
  - `retriever.py` — TF-IDF KB retrieval, chunked on `---` boundaries
  - `llm_client.py` — single Gemini call wrapper (structured, deterministic, retrying)
  - `schemas.py` — Pydantic schemas for structured output
  - `api.py` — FastAPI app (triage + account-brief endpoints)
  - `triage/agent.py` — ticket triage agent
  - `account_brief/summarizer.py` — account brief summariser
  - `eval/` — test cases + eval harness
- **`app_streamlit.py`** — UI
- **`DESIGN_NOTE.md`** — failure modes, trade-offs, scaling
...
latency/quality trade-off made in the account-brief chain,
